...
const generateMyntraJacketMaternityListingFile = (selectedData, csvHeaders) => {
  const sizeMapping = {
    XXS: 'XXS',
    XS: 'XS',
    S: 'S',
  ...
  try {
    ...
    const csvData = selectedData
      .filter(
        (product) =>
          product.styleType.toLowerCase().trim() === 'jacket' ||
          product.styleType.toLowerCase().trim() === 'option'
    ...
    link.download = 'Myntra_Jacket_listing.csv';
...
export { generateMyntraJacketMaternityListingFile };
