...
const generateMyntraShrugMaternityListingFile = (selectedData, csvHeaders) => {
  const sizeMapping = {
    XXS: 'XXS',
    XS: 'XS',
    S: 'S',
  ...
  try {
    ...
    const csvData = selectedData
      .filter(
        (product) =>
          product.styleType.toLowerCase().trim() === 'shrug' ||
          product.styleType.toLowerCase().trim() === 'shrugs' ||
          product.styleType.toLowerCase().trim() === 'cover' ||
          product.styleType.toLowerCase().trim() === 'poncho'
      ...
      .flatMap((product, index) =>
        sizes.map((size) => {
          ...
          const productData = {
            ...
            'Standard Size':
              mappedSize === '1XL' ? 'XL' : mappedSize === '2XL' ? 'XXL' : mappedSize,
    ...
    link.download = 'Myntra_Shrug_listing.csv';
...
export { generateMyntraShrugMaternityListingFile };
