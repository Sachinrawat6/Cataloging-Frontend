...
const generateMyntraTopMaternityListingFile = (selectedData, csvHeaders) => {
  const sizeMapping = {
    XXS: 'XXS',
    XS: 'XS',
    S: 'S',
  ...
  try {
    ...
    const csvData = selectedData
      .filter(
        (product) => product.styleType.toLowerCase().trim() === 'top'
      ...
      .flatMap((product, index) =>
        sizes.map((size) => {
          ...
          const productData = {
            ...
            taxMaterial: '',
            Lining:
              product.lining.toLowerCase().trim().includes('without') ||
              product.lining.toLowerCase().trim().includes('without')
                ? 'NA'
                : 'Has a lining',
    ...
    link.download = 'Myntra_Top_listing.csv';
...
export { generateMyntraTopMaternityListingFile };
