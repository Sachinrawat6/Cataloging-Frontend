...
const generateMyntraTrousersMaternityListingFile = (selectedData, csvHeaders) => {
  const sizeMapping = {
    XXS: 'XXS',
    XS: 'XS',
    S: 'S',
  ...
  try {
    ...
    const csvData = selectedData
      .filter(
        ...
        // &&          product.occasion?.trim()?.toLowerCase()?.split(',')?.includes('maternity')
      )
      .flatMap((product, index) =>
        sizes.map((size) => {
          ...
          const productData = {
            ...
            vendorSkuCode: `6${product.styleNumber}-${product.stylePrimaryColor}-${mappedSize}`,
            vendorArticleNumber: `6${product.styleNumber}` || '',
    ...
    link.download = 'Myntra_Trousers_listing.csv';
...
export { generateMyntraTrousersMaternityListingFile };
