...
const generateMyntraSkirtMaternityListingFile = (selectedData, csvHeaders) => {
  const sizeMapping = {
    XXS: 26,
    XS: 28,
    S: 30,
  ...
  try {
    ...
    const csvData = selectedData
      .filter(
        (product) => product.styleType.toLowerCase().trim() === 'skirt'
      ...
      .flatMap((product, index) =>
        sizes.map((size) => {
          ...
          const productData = {
            ...
            'Slit Detail': '',
            'Add-Ons': 'NA',
            'Fabric Type': product.fabrics[0]?.name || '',
    ...
    link.download = 'Myntra_Skirt_listing.csv';
...
export { generateMyntraSkirtMaternityListingFile };
