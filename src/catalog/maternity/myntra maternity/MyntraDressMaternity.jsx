...
const generateMyntraDressMaternityListingFile = (selectedData, csvHeaders) => {
  const sizeMapping = {
    XXS: 'XXS',
    XS: 'XS',
    S: 'S',
  ...
  try {
    ...
    const csvData = selectedData
      .filter(
        (product) =>
          product.styleType?.toLowerCase() === 'dress' ||
          product.styleType?.toLowerCase() === 'kaftan' ||
          product.styleType.toLowerCase().includes('shirt dress')
      ...
      .flatMap((product, index) =>
        sizes.map((size) => {
          ...
          for (let fabricTypeDress of Fabric_Type_Dresses) {
            if (
              product.fabrics[0]?.name &&
              product.fabrics[0]?.name.trim().toLowerCase() === fabricTypeDress.toLowerCase()
            ) {
          ...
          const productData = {
            ...
            'Fabric 3': '',
            Length: '',
            'Sleeve Length': sleeveMapping[product.sleeveLength.trim()] || '',
            'Knit or Woven': 'Woven',
            Hemline: '',
...
export { generateMyntraDressMaternityListingFile };
