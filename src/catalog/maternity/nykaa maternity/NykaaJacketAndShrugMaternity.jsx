...
const generateNykaaJackeAndShrugeMaternityListing = (selectedData, headers) => {
  ...
  const csvData = selectedData
    .filter(
      (product) =>
        product.styleType?.toLowerCase() === 'coat' ||
        product.styleType?.toLowerCase() === 'jacket' ||
        product.styleType?.toLowerCase() === 'shrug' ||
        product.styleType?.toLowerCase() === 'shrugs' ||
        product.styleType?.toLowerCase() === 'hoodie'
    )
    .flatMap((product) =>
      sizes.map((size) => {
        ...
        product.stylePrimaryColor =
          colorMapping[product.stylePrimaryColor] || product.stylePrimaryColor;
        ...
        const occasionMapping = {
          ...
          'Evening, Party, Casual, Office, Daily, Festival, Night': 'Evening Wear',
        ...
        return {
          'Vendor SKU Code': `6${product.styleNumber}-${product.stylePrimaryColor}-${mappedSize}`, // Use mapped size
          Gender: 'Women',
          'Brand Name': 'Qurvii',
          'Style Code': `6${product.styleNumber}` || '',
          ...
          'Type of Work': '',
          'Jackets and Coats Subcategory': `${jacketSubcategoryMapping[product.styleType]}s ` || '',
          Closure: product.closure.toLowerCase().trim().includes('not applicable')
            ? 'Other'
            : product.closure || '',
          ...
          Age: '',
          'Bust For Body (Inches)': sizeData[size]?.[1] ?? '',
          'Chest For Body (Inches)': sizeData[size]?.[2] ?? '',
          'Waist For Body (Inches)': sizeData[size]?.[5] ?? '',
          'Bust for Garment (Inches)': sizeData[size]?.[1] ?? '',
          'Chest for Garment (Inches)': sizeData[size]?.[2] ?? '',
          'Waist for Garment (Inches)': sizeData[size]?.[5] ?? '',
          'Shoulder for Garment (Inches)': sizeData[size]?.[0] ?? '',
          'Sleeve Length (Inches)': product.measurements.sleeveLengthXS || 0,
          'Shoulder For Body (Inches)': sizeData[size]?.[0] ?? '',
          'Length For Body (Inches)': sizeData[size]?.[3] ?? '',
          'Length (Inches)': addFrontLength(product),
        };
      })
    );
...
export { generateNykaaJackeAndShrugeMaternityListing };
