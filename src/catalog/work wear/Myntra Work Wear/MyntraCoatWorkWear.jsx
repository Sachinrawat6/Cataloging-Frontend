...
const generateMyntraCoatWorkWearListingFile = (selectedData, csvHeaders) => {
  const sizeMapping = {
    XXS: 'XXS',
    XS: 'XS',
    S: 'S',
  ...
  try {
    ...
    const csvData = selectedData
      .filter((product) => product.styleType.toLowerCase().trim() === 'coat')
      .flatMap((product, index) =>
        sizes.map((size) => {
          ...
          const productData = {
            ...
            vendorSkuCode: `7${product.styleNumber}-${product.stylePrimaryColor}-${mappedSize}`,
            vendorArticleNumber: `7${product.styleNumber}` || '',
...
export { generateMyntraCoatWorkWearListingFile };
