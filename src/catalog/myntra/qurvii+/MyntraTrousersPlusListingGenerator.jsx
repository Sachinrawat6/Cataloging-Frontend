...
const generateMyntraTrousersPlusListingFile = (selectedData, csvHeaders) => {
  // const csvHeaders = [
  //     "styleId",
  //     "styleGroupId",
  //     "vendorSkuCode",
  ...
  try {
    ...
    const csvData = selectedData
      // .filter((product) => product.styleType.toLowerCase().trim() === "pant" || product.styleType.toLowerCase().trim() === "plazzo" || product.styleType.toLowerCase().trim() === "shorts" || product.styleType.toLowerCase().trim() === "short" || product.styleType.toLowerCase().trim() === "trouser")
      .filter(
        (product) =>
          product.styleType.toLowerCase().trim() === 'pant' ||
          product.styleType.toLowerCase().trim() === 'joggers' ||
          product.styleType.toLowerCase().trim() === 'plazzo' ||
          product.styleType.toLowerCase().trim() === 'shorts' ||
          product.styleType.toLowerCase().trim() === 'short' ||
          product.styleType.toLowerCase().trim() === 'trouser'
      )
        ...
        sizes.map((size) => {
          ...
          for (let fabric of Fabric_Dresses) {
            if (product.fabrics[0]?.name.toLowerCase().trim() === fabric) {
              ...
              if (
                product.fabrics[0]?.name.toLowerCase().trim().includes('crepe') ||
          ...
          for (let fabricTypeDress of Fabric_Type_Dresses) {
            if (
              product.fabric &&
              product.fabric.trim().toLowerCase() === fabricTypeDress.toLowerCase()
            ) {
          ...
          for (const occasion of Occasion_Values) {
            if (
              product.occasion &&
              product.occasion.toLowerCase().trim().includes(occasion.toLowerCase())
            ) {
          ...
          for (const neckline of Neckline_Values) {
            if (
              product.neckStyle &&
              product.neckStyle.toLowerCase().trim().includes(neckline.toLowerCase())
            ) {
              ...
              product.neckStyle.toLowerCase().trim() === 'v-neck' ||
              product.neckStyle.toLowerCase().trim() === 'v neck'
            ) {
          ...
          const productData = {
            ...
            brand: 'Qurvii',
            'Manufacturer Name and Address with Pincode':
              'B-149, Sector 6, Noida, UP 201301, India',
            'Packer Name and Address with Pincode':
              'Qurvii, B-149, Sector 6, Noida, UP 201301, India',
            'Importer Name and Address with Pincode': '',
            ...
            'Standard Size': standardSizeMapping[mappedSize],
            ...
            'Garment Waist ( Inches )': sizeData[size]?.[3] ?? '',
            'Inseam Length ( Inches )': '',
            'Skirt Length ( Inches )': '',
            'To Fit Waist ( Inches )': sizeData[size]?.[3] ?? '',
            'Hips ( Inches )': '',
            'Outseam Length ( Inches )': '',
            'Rise ( Inches )': '',
            'Thigh ( Inches )': '',
    ...
    link.download = 'Myntra_Trousers_Plus_listing.csv';
...
export { generateMyntraTrousersPlusListingFile };
