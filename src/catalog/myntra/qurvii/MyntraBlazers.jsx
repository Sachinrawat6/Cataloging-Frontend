...
const generateBlazersListingFile = (selectedData, csvHeaders) => {
  const sizeMapping = {
    XXS: 'XXS',
    XS: 'XS',
    S: 'S',
  ...
  try {
    ...
    const csvData = selectedData
      .filter(
        (product) =>
          product.styleType.toLowerCase().trim() === 'blazers' ||
          product.styleType.toLowerCase().trim() === 'blazer'
      )
      .flatMap((product, index) =>
        sizes.map((size) => {
          ...
          const productData = {
            ...
            styleNote: '',
            materialCareDescription: product.fabrics[0]?.name?.trim() || '',
            ...
            'Main Trend': '',
            Character: '',
            'Fabric Type': product.fabricType || 'NA',
            'Sport Team': '',
            'Number of Items': 1,
...
export { generateBlazersListingFile };
